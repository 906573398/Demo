...
class FormBuilder
{
    ...
    public function label($name, $value = null, $options = [])
    {
        ...
        return '<label class="checkbox-label" for="' . $name . '"' . $options . '>' . $value . '</label>';
    }
...
EOD;
    ...
    public function checkboxs($name, $list, $checked, $options = [])
    {
        ...
        foreach ($list as $k => $v) {
            $options['id'] = "{$name}-{$k}";
            $options['name'] = "{$name}[{$k}]";
            $element = '<label class="checkbox-label">';
            $element .= '<input value="' . $k . '" ' . $this->attributes($options) . ' type="checkbox" class="custom-control-input">';
            $element .= '<span class="label-icon"></span>';
            $element .= '<span class="label-text">' . $v . '</span>';
            $element .= '</label>';
            $html[] = $element;
        }
     ...
     * @return string
     */
    public function radios($name, $list, $checked = null)
    {
        $html = '';
        foreach ($list as $k => $v) {
            $html .= '<label class="radio-label">';
            $html .=  '<input id="' . ($name . $k) . '" ' . ($k == $checked ? 'checked' : '') . ' value="' . $k . '"
                    name="' . $name . '" type="radio" class="custom-control-input">';
            $html .= '<span class="label-icon"></span><span class="label-text">' . $v . '</span></label>';
        }
        return $html;
    }
     ...
     * 
     */
    public function image($name = null, $value, $options = [])
    {
        $option = $this->attributes($options);
        $html = <<<EOL
            <div class="upload-group">
            <div class="input-group">
                <input class="form-control file-input" name="$name"
                    value="$value" $option >
    ...
    public function images($name = null, $value, $hiden_upload = false, $hiden_select = false)
    {
        $value = explode(',', $value);
        $upload = $hiden_upload ? '' : '<span class="input-group-btn">
                                            <a class="btn btn-secondary upload-file" href="javascript:" data-toggle="tooltip" data-placement="bottom" title="上传文件">
                                                <span class="iconfont icon-cloudupload"></span>
                                            </a>
                                        </span>';
        $select = $hiden_select ? '' : '<span class="input-group-btn">
        ...
        $html = <<<EOL
            ...
            <div class="upload-preview-list sort-list">
        EOL;
        if (count($value) > 0) {
            foreach ($value as $key => $item) {
                $html .= '
                    <div class="upload-preview text-center" flex="cross:center">
                        <input type="hidden" class="file-item-input" name="' . $name . '[]" value="' . $item . '">
            ...
            $html .= '<div class="upload-preview text-center">
                    <input type="hidden" class="file-item-input" name="' . $name . '[]">
                    <span class="file-item-delete">&times;</span>
                    <span class="upload-preview-tip">750&times;750</span>
    ...
    protected function uploader($name = null, $value, $inputAttr = [], $uploadAttr = [], $chooseAttr = [], $previewAttr = [])
    {
        ...
        $chooseBtn = $choose ? $this->button('<i class="fa fa-list"></i> 选择', array_merge($options, $chooseAttr)) : '';
